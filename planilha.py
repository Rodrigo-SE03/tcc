...
def tab_consumo_por_carga(cargas,writer,grupo,h_p):
    ...
    worksheet.set_column(0, max_col - 1, 12)
    estilos.consumo_equip_style(worksheet,workbook,grupo)
    worksheet.autofit()
    return equip_dict
#--------------------------------------------------------------------------------------------------------
...
def tab_consumo(itens,writer,categoria,h_p,equip_dict,tarifas_dict,dias):  
    ...
    worksheet.add_table(0, 0, max_row, max_col - 1, {"columns": column_settings})
    worksheet.set_column(0, max_col - 1, 12)
    custos_mensais = estilos.custos(custo=custo,writer=writer,categoria=categoria,workbook=workbook,worksheet=worksheet,dias=dias,tarifas_dict=tarifas_dict)
...
def tab_reativos(categoria,consumo_dict,h_p,tarifas_dict,writer,dias):
    ...
    demanda_p = max(consumo_dict['Potência P - kW'])
    if categoria == 'Verde':
        t_cfp = tarifas_dict['verde'][0]
        t_cp = tarifas_dict['verde'][1] 
        t_dem = tarifas_dict['verde'][2]
    else:
        t_cfp = tarifas_dict['azul'][0]
        t_cp = tarifas_dict['azul'][1] 
        t_demfp = tarifas_dict['azul'][2]
        t_demp = tarifas_dict['azul'][3]
    ...
    for p in periodo:
        ...
        if ind[j] !=0 or cap[j] != 0:
            if ind[j] > 0.92 or cap[j] >0.92:
                ...
                rs[j] = math.fabs(consumo) * ((0.92/(ind[j] if ind[j] != 0 else cap[j]))-1) * (t_cfp if c_fp[j] != 0 else t_cp)
        ...
        if dem_kw[j] > maior_fp and dem_fp[j] != 0 and categoria == 'Azul':
                ...
                if ind[j] != 0 or cap[j] != 0:
                    ...
                    else: demr_fp = (maior_fp-demanda_fp)*t_demfp
        ...
        if dem_kw[j] > maior_p and dem_p[j] != 0 and categoria == 'Azul':
            ...
            if ind[j] != 0 or cap[j] != 0:
                ...
                else: demr_p = (maior_p-demanda_p)*t_demp
    ...
    estilos.tabela_reativos(categoria=categoria,demr=demr,tabela_dict=tabela_dict,writer=writer,consumo_mes=consumo_mes)
...
def comparativo_gpb(m_results_C,m_results_B,grupo,writer):
    ...
    estilos.comparativo_style(grupo=grupo,comp_dict=comp_dict,writer=writer)
#--------------------------------------------------------------------------------------------------------
